...
try {
    if (Buffer && Buffer.from) { const hasBufferType = true; }
} catch (err) {
    hasBufferType = false;
}
 ...
 * with the rest of the ObjectID zeroed out. Used for comparisons or sorting the ObjectID.
 *
 * @method
 ...
 * @return {ObjectID} return the created ObjectID
 */
ObjectID.createFromHexString = function createFromHexString(inputString) {
    // Throw an error if it's not a valid setup
    if (typeof inputString === "undefined" || (inputString != null && inputString.length !== 24)) {
        throw new Error(
            "Argument passed in must be a single String of 12 bytes or a string of 24 hex characters",
        );
    }
